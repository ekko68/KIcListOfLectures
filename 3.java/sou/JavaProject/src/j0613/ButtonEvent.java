...
import java.awt.*;//그래픽
import java.awt.event.*;//~Listener,~Adapter(event패키지)
...
public class ButtonEvent extends Frame implements ActionListener{
	...
	public ButtonEvent() {
		...
		this.setLayout(new GridLayout(5,1,3,3));//5행 1열
		b1=new Button("시작");
		b2=new Button("고");
		b3=new Button("백");
		b4=new Button("점프");
		tf1=new TextField(" ");//빈문자열에 해당하는 텍스트입력창(TextArea)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(s.equals("시작")) {//시작버튼을 눌렀다면
			b1.setBackground(Color.red);//색깔은 Color->정적상수
			setTitle(s);
		}else if(s.equals("고")) { //b2->yellow
			b2.setBackground(Color.yellow);//색깔은 Color->정적상수
			setTitle(s);
		}else if(s.equals("백")) { //b3->blue
			b3.setBackground(Color.blue);//색깔은 Color->정적상수
			setTitle(s);
		}else { //점프->b4->orange
			b4.setBackground(Color.orange);//색깔은 Color->정적상수
			setTitle(s);
		}
	}
