...
//입출력->BufferedReader,InputStreamReader,PrintWriter~
...
public class ChatGUIServer {
	...
	ServerThread st; //쓰레드 기능과 따로 클래스
	//-------------------------------------------------
	public ChatGUIServer() throws Exception{ //생성자->초기화
		...
		if("".equals(service)) {//정상적으로 port속성을 못 불러오는 경우
			service="5432";
		}
		...
		try {
			//ss=new ServerSocket(5432);//책 7777=>포트번호
			ss=new ServerSocket(port);
			System.out.println("ss=>"+ss);
			System.out.println("채팅서버 가동중.....");
			//무한대기->언제든지 접속이 가능한 상태로 만들기 위해서
			while(true) {
				s=ss.accept();//요청을 받아들인다.->Socket(ip주소,5432)
...
class ServerThread extends Thread{
	...
	Socket s;//클라이언트와 통신
	//추가(has a 관계)
	ChatGUIServer cg;//ChatGUIServer cg=new ChatGUIServer(); X
	//--------------------------------
	//입출력 스트림객체 필요
